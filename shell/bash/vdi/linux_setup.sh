...
if [ Z"$theos" = Z"centos" ] || [ Z"$theos" = Z"redhat" ] || [ Z"$theos" = Z"fedora" ]; then
		...
		if [ ! -f $HOME/resume-after-reboot ]; then
				...
				which dnf >& /dev/null
				if [ $? -eq 1 ]; then
						sudo yum install -y epel-release dnf && sudo dnf upgrade -y epel-release && sudo dnf config-manager --enable cr; else
						sudo dnf install -y epel-release && sudo dnf upgrade -y epel-release && sudo dnf config-manager --enable cr;
				fi
				
				# Change repo release version
				# https://www.webscalability.com/blog/2020/01/error-failed-to-synchronize-cache-for-repo-appstream-for-centos-8/
				if [ Z"$theos" = Z"centos" ]; then
						if [[ $major -eq 8 && $minor -eq 0 ]]; then
								colorecho "changing releasever in /etc/yum.repos/d to 8" 3
								cd /etc/yum.repos.d && sudo sed -i.bak -e 's/$releasever/8/g' *.repo && cd $HOME
						fi
				fi
				...
				if [ Z"$theos" = Z"centos" ]; then
						if [[ $major -eq 8 && $minor -eq 0 ]]; then
								# system upgrade called after prettyping install
								colorecho "Skipping update on $theos $full for now..." 3
				...
				colorecho "snaps fail to install during script exec...install snap apps manually..." 3 && sleep 5
				...
				# Install Octant
				colorecho "it will take a while to install octant, be patient..." 3 && sleep 5 && brew install octant
				...
				# Upgrade CentOS 8.0 to 8.1 after prettyping install
				if [ Z"$theos" = Z"centos" ]; then
						if [[ $major -eq 8 && $minor -eq 0 ]]; then
								sudo dnf upgrade -y && sudo dnf autoremove -y && sudo dnf clean all
						fi
				...
				# Revert change repo release version
				if [ Z"$theos" = Z"centos" ]; then
						if [[ $major -eq 8 && $minor -eq 0 ]]; then
								cd /etc/yum.repos.d && sudo sed -i -e 's/8/$releasever/g' *.repo && sudo rm -rf *.bak && cd $HOME
						fi
				fi
				...
				sed -i 's/plugins=(git)/plugins=(git zsh-syntax-highlighting pks)/' ~/.zshrc
				sed -i '$a \\nexport PATH=$PATH:"/home/linuxbrew/.linuxbrew/bin"' ~/.zshrc
				sed -i '$a \\nexport PATH=$PATH:"/snap/bin"' ~/.zshrc
				sed -i '$a \\export PATH=$PATH:"/var/lib/snapd/snap/bin"' ~/.zshrc
				sed -i '$a \\nPOWERLEVEL9K_TIME_BACKGROUND=blue' ~/.zshrc
				sed -i '$a \\POWERLEVEL9K_DATE_BACKGROUND=cyan' ~/.zshrc
				sed -i '$a \\POWERLEVEL9K_DATE_FORMAT=%D{%m.%d.%y}' ~/.zshrc
				sed -i '$a \\POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(status root_indicator background_jobs history time date)\n' ~/.zshrc
		...
		if [ ! -f $HOME/resume-after-reboot ]; then
				...
				# Install snaps
				sudo snap install helm --classic && sudo snap install atom --classic
				sudo snap install notepadqq && sudo snap install p7zip-desktop && sudo snap install snap-store \
				&& sudo snap install canonical-livepatch && sudo snap install termius-app && sudo snap install docker \
				&& sudo snap install 1password-linux && sudo snap install remmina
				...
				sed -i '$a \\nexport PATH=$PATH:"/home/linuxbrew/.linuxbrew/bin"' ~/.zshrc
				sed -i '$a \\nexport PATH=$PATH:"/snap/bin"' ~/.zshrc
				sed -i '$a \\nPOWERLEVEL9K_TIME_BACKGROUND=purple' ~/.zshrc
				sed -i '$a \\POWERLEVEL9K_DATE_BACKGROUND=orange1' ~/.zshrc
				sed -i '$a \\POWERLEVEL9K_DATE_FORMAT=%D{%m.%d.%y}' ~/.zshrc
				sed -i '$a \\POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(status root_indicator background_jobs history time date)\n' ~/.zshrc
				sed -i '$a \tmux\nalias c="clear"\nalias k="kubectl"\nalias ping="prettyping"\nalias w="watch -n1"\nalias cat="bat"' ~/.zshrc
				sed -i '$a \\nif [ $commands[kubectl] ]; then source <(kubectl completion zsh); fi' ~/.zshrc
